...
head = ["it","accepted","max_fraction_of_larvae_spB","start","end","duration","max_death_rate_hybrid","rseed"]
...
while abc_iteration < target_samples:
	...
	[start_point, end_point] = np.sort(np.random.uniform(0,n_time_steps, 2))  # mid point of logistic decrease in influx of spB larvae
	...
	print "\nrandom seed:", rseed, start_point, end_point, max_fraction_of_larvae_spB
	...
	if accepted>0: 	
		abc_iteration+=1
		log_state = map(str, (abc_iteration,abc_iteration/float(abc_iter), max_fraction_of_larvae_spB,start_point, end_point, end_point-start_point,max_death_rate_hybrid,rseed )) + map(str,summary_stats)
