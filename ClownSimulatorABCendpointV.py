...
logfile_name = "%s/abc_window%s.log" % (w_dir,args.out)
mcmc_logfile = open(logfile_name , "w",0) 
head = ["it","accepted","max_fraction_of_larvae_spB","durationHybridization", "timeSinceHybridization","max_death_rate_hybrid","rseed"]
#  fraction of spB mtDNA, median admixture (>80% spA), admixt individuals (>80% spA) with spB mtDNA
head = head + ["median_nDNAadmixture","nDNA_80spA","nDNA_80spB","mtDNA","Median_nDNA_in_mtDNAspB","Min_nDNA_in_mtDNAspB"]
...
while abc_iteration < target_samples:
	...
	duration_hybridization = np.random.randint(0,1500)
	duration_after_hybridization = np.random.randint(0,1500)
	
	start_point, end_point = 0, duration_hybridization  # mid point of logistic decrease in influx of spB larvae
	steepness = 1 #np.random.uniform(0,1)  # steepness of logistic decrease in influx of spB larvae
	
	n_time_steps = duration_hybridization+duration_after_hybridization
	...
	print "\nrandom seed:", rseed, end_point, duration_after_hybridization, max_fraction_of_larvae_spB
	...
	def get_plot(args):	
		[spB_nucleus, spB_mitoc, nDNA_indApure, nDNA_indBpure, hybridization_list, mtDNA, nDNAadmixture,median_nDNAadmixture] = args
		...
		plt.plot(x_axis,median_nDNAadmixture,color= "purple")
		plt.gca().set_ylim(Ylim)
	...
	for time_step in range(n_time_steps+1):
		...
		if time_step >= start_point and time_step < end_point: 
			fraction_of_larvae_spB = max_fraction_of_larvae_spB
		else: fraction_of_larvae_spB = 0
		...
		median_nDNAadmixture.append(np.median(nDNAadmixture))
		...
		# kill simulation if going nowhere
		mtDNA_temp = mtDNA+0
		mtDNA_temp[mtDNA_temp<=1] = 0
		mtDNA_temp[mtDNA_temp>1] = 1
		if time_step > end_point+1:
			if np.size(mtDNA_temp[mtDNA_temp<=1]) == 0 or np.size(mtDNA_temp[mtDNA_temp<=1]) == 1: 
				accepted=0
				break
		...
		if plot:
			if time_step % 500 ==0 and time_step>0 and accepted >= 0:
				fig, legend = get_plot([spB_nucleus, spB_mitoc, nDNA_indApure, nDNA_indBpure, hybridization_list, mtDNA, nDNAadmixture,median_nDNAadmixture])
				figures_list.append(fig)
	
	#print(summary_stats)
	# check acceptance
	if accepted==1:
		if np.sum(mtDNA_temp)/total_pop_size > 0 and np.median(nDNAadmixture)< 0.2:
			spA80 = len(nDNAadmixture[nDNAadmixture<0.2])/float(len(nDNAadmixture))			
			spB80 = len(nDNAadmixture[nDNAadmixture>0.8])/float(len(nDNAadmixture))			
			summary_stats = [np.median(nDNAadmixture),spA80,spB80, np.mean(mtDNA_temp), np.median(nDNAadmixture[mtDNA_temp==1]), np.min(nDNAadmixture[mtDNA_temp==1])]
			print np.sum(mtDNA_temp)/total_pop_size,  np.median(nDNAadmixture), summary_stats
		else: 
			accepted=0
			print np.median(nDNAadmixture), np.sum(mtDNA_temp)/total_pop_size
			fig = 0
			figures_list = []
	else: accepted=0
	...
	if accepted>0: 	
		...
		#  
		if len(plot_end_res_list)<=plot_end_res:
			fig_end, legend = get_plot([spB_nucleus, spB_mitoc, nDNA_indApure, nDNA_indBpure, hybridization_list, mtDNA, nDNAadmixture,median_nDNAadmixture])
			plot_end_res_list.append(fig_end)
		
			if len(plot_end_res_list)==plot_end_res:
				file_name = "%s/end_res%s.pdf" % (w_dir,args.out)
				pdf = matplotlib.backends.backend_pdf.PdfPages(file_name)
		
				for fig_end in plot_end_res_list: ## will open an empty extra figure :(
				    pdf.savefig( fig_end )
				pdf.close()
